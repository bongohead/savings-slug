...
function getAJAX(model, toScript, fromAjax, timeout = 10000) {
...
function getFetch(model, toScript, fromAjax, timeout = 10, verbose = false) {
	...
	return new Promise ((resolve, reject) => 
		...
		.fail(function(res) {
			if (verbose) console.log('Fetch Error 1', res);
		})
		.always(function(res) {
			if (verbose) console.log('Fetch Time: '+ (Date.now() - timerStart));
		})
		.done(function(res) {
			if (!isJson(res)) {
				if (verbose) console.log('Fetch Error 2: ', res);
				resolve();
			}
...
/* Group an array of objects by an object property
 * @example groupBy({type: 'orange', title: 1}, {type: 'orange', title: 2'}, {type: 'apple', title: 2}, 'apple') will give
 * {orange: array[2], apple; array[1]}
 *
 */
...
/* Get an array of dates from a date range and interval
 *
 *
 *
 */
